...
try {
    $conectar = new mysqli($data['db_config']['servidordb'] , $data['db_config']['userdb'], $data['db_config']['passworddb'], $data['db_config']['nombredb']); //Detalles BD 
...
for ($i=0; $i < count($boards); $i++)
			...
			if($lists[$k]['closed']==true)
					{
						...
						{ 	 
							if(isset($actions[$p]['data']['old']['closed']))//SI LA ACCION CORRESPONDE A ARCHIVAR
							{
								...
								if($idLista == $id && $accionCerrar == false ){ //NO ENTRA A ESTE IF
										$fechaArchivado = explode("T", $actions[$p]['date']); //$fecha [0] es la fecha
 ...
 //***************************************************** RECORRER ACCIONES QUE CORRESPONDEN A CREAR  LISTA ***************************************************
	...
	//$urlAcciones ='https://trello.com/1/boards/'.$boards[$i]['shortLink'].'/actions&key='.$data['trello_config']['apikey'].'&token='.$data['trello_config']['token'].'';
	...
	if($actionsC[$l]['type'] == 'createList')
		{			
			...
			$fecha = explode("T", $actionsC[$l]['date']); //$fecha [0] es la fecha
